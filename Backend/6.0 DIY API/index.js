...
app.post("/jokes", (req, res) => {
  const newJoke = { id: jokes.length + 1, ...req.body };
  jokes.push(newJoke);
  res.json(newJoke);
});
...
app.put("/jokes/:id", (req, res) => {
  ...
  currentJoke.jokeText = req.body.text;
  currentJoke.jokeType = req.body.type;
  jokes[jokes.findIndex((joke) => joke.id === currentJoke.id)] = currentJoke;
...
app.delete("/jokes/:id", (req, res) => {
  ...
  if (index === -1) {
    res
      .status(404)
      .json({
        error: "No jokes exist for the mentioned id. Nothing was deleted.",
      });
  } else {
...
app.delete("/all", (req, res) => {
  if(req.query.apiKey === masterKey) {
    ...
    res.status(404).json({error: "You are not authorised to perform this operation."})
  }
});
